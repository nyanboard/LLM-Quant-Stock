...
路由注册和中间件配置。screener 路由已注册到 /api/v1/screener。
...
from api.routers import stock, backtest, agent, config as config_router, screener
...
async def startup():
    ...
    logger.info("数据缓存初始化完成，screener 路由已就绪")
